    ...
    public InventorySnapshot(Player player, Match match) {
        ...
        if (potionMatch) {
            ...
            this.inventoryUI.setItem(47, new InventoryUI.EmptyClickableItem(ItemUtil.reloreItem(
                    ItemUtil.createItem(Material.POTION, CC.YELLOW + "Pociones de Curacion: " + CC.SECONDARY + potCount, potCount, (short) 16421),
                    CC.YELLOW + "Pociones gastadas: " + CC.SECONDARY + playerData.getMissedPots())));
        } else if (soupMatch) {
            ...
            this.inventoryUI.setItem(47, new InventoryUI.EmptyClickableItem(ItemUtil.createItem(
                    Material.MUSHROOM_SOUP, CC.YELLOW + "Sopas restantes: " + CC.SECONDARY + soupCount, soupCount, (short) 16421)));
        }

        this.inventoryUI.setItem(48,
                new InventoryUI.EmptyClickableItem(ItemUtil.createItem(Material.SKULL_ITEM, CC.YELLOW + "Corazones: "
                        + CC.SECONDARY + MathUtil.roundToHalves(health / 2.0D) + " / 10 ❤", (int) Math.round(health / 2.0D))));

        this.inventoryUI.setItem(49,
                new InventoryUI.EmptyClickableItem(ItemUtil.createItem(Material.COOKED_BEEF, CC.YELLOW + "Comida: "
        ...
        this.inventoryUI.setItem(50,
                new InventoryUI.EmptyClickableItem(ItemUtil.reloreItem(
                        ItemUtil.createItem(Material.BREWING_STAND_ITEM, CC.YELLOW + "Efectos de Pociones", potionEffectStrings.size())
                        , potionEffectStrings.toArray(new String[]{}))));

        this.inventoryUI.setItem(51, new InventoryUI.EmptyClickableItem(
                ItemUtil.reloreItem(
                        ItemUtil.createItem(Material.DIAMOND_SWORD, CC.YELLOW + "Estadisticas"),
                        CC.PRIMARY + "Combo mas Largo: " + CC.SECONDARY + playerData.getLongestCombo() + " Hit" +
                                (playerData.getLongestCombo() > 1 ? "s" : ""),
                        CC.YELLOW + "Hits Totales: " + CC.SECONDARY + playerData.getHits() + " Hit" + (playerData.getHits() > 1 ? "s" : ""))));

        if (!match.isParty()) {
            for (int i = 0; i < 2; i++) {
                this.inventoryUI.setItem(i == 0 ? 53 : 45, new InventoryUI.AbstractClickableItem(
                        ItemUtil.reloreItem(ItemUtil.createItem(Material.PAPER, CC.YELLOW + "Ver otro Inventario"),
                                CC.YELLOW + "Click para ver el otro Inventario")) {
                    @Override
